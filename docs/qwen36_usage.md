...
| `FLASHRT_QWEN36_DFLASH_WINDOW_SEED` | Optional | `1` | Seed the per-token window from the prompt tail during DFlash prefill. Disable for benchmarks built from verbatim-repeated text. |
| `FLASHRT_QWEN36_DFLASH_STEP_SAVES` | Optional | `1` on Thor, `0` on RTX | Grow DFlash verify rollback checkpoints to K+1 rows and recover partial accepts with state copies. On 31 GB RTX this does not fit when MTP and DFlash are resident together. |
| `FLASHRT_QWEN36_DFLASH_CHUNK_SAVES` | Optional | `0` | RTX opt-in chunk-saves verify kernels for DFlash linear-attention layers. Requires `FLASHRT_QWEN36_DFLASH_STEP_SAVES=1`. |
