...
#endif
#ifdef FLASHRT_HAVE_W4A16_PACKED
...
#ifdef FLASHRT_HAVE_W4A16_PACKED
    m.def("w8a16_rowwise_matvec_bf16",
        [](uintptr_t x, uintptr_t weight, uintptr_t scale, uintptr_t out,
           int N, int K, uintptr_t stream) -> int {
            return flash_rt::kernels::w8a16_rowwise_matvec_bf16(
                to_ptr(x), to_ptr(weight), to_ptr(scale), to_ptr(out),
                N, K, to_stream(stream));
        },
        py::arg("x"), py::arg("weight"), py::arg("scale"), py::arg("out"),
        py::arg("N"), py::arg("K"), py::arg("stream") = 0);

    m.def("int8_rowwise_gather_bf16",
        [](uintptr_t ids, uintptr_t weight, uintptr_t scale, uintptr_t out,
           int rows, int table_rows, int K, uintptr_t stream) -> int {
            return flash_rt::kernels::int8_rowwise_gather_bf16(
                to_ptr(ids), to_ptr(weight), to_ptr(scale), to_ptr(out),
                rows, table_rows, K, to_stream(stream));
        },
        py::arg("ids"), py::arg("weight"), py::arg("scale"), py::arg("out"),
        py::arg("rows"), py::arg("table_rows"), py::arg("K"),
        py::arg("stream") = 0);
