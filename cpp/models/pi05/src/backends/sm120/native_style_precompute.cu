...
modalities::Status NativeStylePrecomputer::run(
    ...
    if (!time_output || !style_attn || !style_ffn || !style_final ||
        !scratch_a || !scratch_b || time_output->shape.size() != 3 ||
        style_attn->shape.size() != 4 || style_ffn->shape != style_attn->shape ||
        style_final->shape.size() != 3) {
        return invalid("native style workspace layout is invalid");
    }
    ...
    if (time_output->shape[2] != 1024 || style_attn->shape[0] != steps ||
        ...
        return invalid("native style workspace shape is invalid");
    }
    ...
    if (!weight_shape(weights, "decoder_time_embeds",
                      ...
                      &time_source) ||
        !weight_shape(weights, "decoder_time_mlp_in_w", {1024, 1024}, &time_in_w) ||
        !weight_shape(weights, "decoder_time_mlp_in_b", {1024}, &time_in_b) ||
        !weight_shape(weights, "decoder_time_mlp_out_w", {1024, 1024}, &time_out_w) ||
        !weight_shape(weights, "decoder_time_mlp_out_b", {1024}, &time_out_b) ||
        !weight_shape(weights, "decoder_final_norm_mod_w", {1024, 3072}, &final_w) ||
        !weight_shape(weights, "decoder_final_norm_mod_b", {3072}, &final_b)) {
        return invalid("native style global weights are incomplete");
    }
    const cudaStream_t cuda_stream = reinterpret_cast<cudaStream_t>(stream);
    const std::uintptr_t native_stream = stream;
    for (int step = 0; step < steps; ++step) {
        ...
        modalities::Status st = driver_->bf16_nn(
            ...
            frt_buffer_dptr(scratch_a->buffer), 1, 1024, 1024,
            native_stream);
        if (!st.ok_status()) return st;
        st = driver_->add_bias_bf16(
            frt_buffer_dptr(scratch_a->buffer),
            frt_buffer_dptr(time_in_b->buffer), 1, 1024, native_stream);
        if (!st.ok_status()) return st;
        st = driver_->silu_bf16(frt_buffer_dptr(scratch_a->buffer), 1024,
                                native_stream);
        if (!st.ok_status()) return st;
        st = driver_->bf16_nn(
            frt_buffer_dptr(scratch_a->buffer),
            frt_buffer_dptr(time_out_w->buffer),
            frt_buffer_dptr(scratch_b->buffer), 1, 1024, 1024,
            native_stream);
        if (!st.ok_status()) return st;
        st = driver_->add_bias_bf16(
            frt_buffer_dptr(scratch_b->buffer),
            frt_buffer_dptr(time_out_b->buffer), 1, 1024, native_stream);
        if (!st.ok_status()) return st;
        st = driver_->silu_bf16(frt_buffer_dptr(scratch_b->buffer), 1024,
                                native_stream);
        if (!st.ok_status()) return st;
        ...
        for (int row = 0; row < chunk; ++row) {
            const cudaError_t rc = cudaMemcpyAsync(
                ...
                cuda_stream);
            if (rc != cudaSuccess) return backend("time style expansion failed");
        }
        ...
        for (int layer = 0; layer < 18; ++layer) {
            ...
            if (!weight_shape(weights, "decoder_pre_attn_norm_mod_w_" + suffix,
                              {1024, 3072}, &attn_w) ||
                !weight_shape(weights, "decoder_pre_attn_norm_mod_b_" + suffix,
                              {3072}, &attn_b) ||
                !weight_shape(weights, "decoder_pre_ffn_norm_mod_w_" + suffix,
                              {1024, 3072}, &ffn_w) ||
                !weight_shape(weights, "decoder_pre_ffn_norm_mod_b_" + suffix,
                              {3072}, &ffn_b)) {
                return invalid("native style layer weights are incomplete");
            }
            ...
            st = driver_->bf16_nn(expanded, frt_buffer_dptr(attn_w->buffer),
                                  attn_target, chunk, 3072, 1024,
                                  native_stream);
            if (!st.ok_status()) return st;
            st = driver_->add_bias_bf16(attn_target,
                                        frt_buffer_dptr(attn_b->buffer),
                                        chunk, 3072, native_stream);
            if (!st.ok_status()) return st;
            st = driver_->bf16_nn(expanded, frt_buffer_dptr(ffn_w->buffer),
                                  ffn_target, chunk, 3072, 1024,
                                  native_stream);
            if (!st.ok_status()) return st;
            st = driver_->add_bias_bf16(ffn_target,
                                        frt_buffer_dptr(ffn_b->buffer),
                                        chunk, 3072, native_stream);
            if (!st.ok_status()) return st;
        }
        ...
        st = driver_->bf16_nn(expanded, frt_buffer_dptr(final_w->buffer),
                              final_target, chunk, 3072, 1024,
                              native_stream);
        if (!st.ok_status()) return st;
        st = driver_->add_bias_bf16(final_target,
                                    frt_buffer_dptr(final_b->buffer),
                                    chunk, 3072, native_stream);
        if (!st.ok_status()) return st;
    }
    ...
    return rc == cudaSuccess
               ...
               : backend("native style precompute synchronization failed");
}
