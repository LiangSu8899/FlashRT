...
void record_layer(void* user, void* stream) {
    auto* args = static_cast<CaptureArgs*>(user);
    args->recorded = args->forward
        ->encoder_layer(0, *args->weights, args->workspace, args->attention,
...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cerr << "usage: pi05_native_encoder_layer_probe CHECKPOINT OUTPUT\n";
        return 2;
    }
    ...
    flashrt::modalities::Status st = materializer.materialize_encoder_layer(0);
    ...
    frt_graph graph = frt_graph_create(ctx, "native_encoder_layer", 712);
    ...
    if (frt_graph_capture(graph, 712, record_layer, &capture) != FRT_OK ||
        !capture.recorded) {
    ...
    std::vector<std::uint16_t> output(712 * 2048);
    if (cudaMemcpy(output.data(), frt_buffer_dptr(encoder_x->buffer),
    ...
    file.write(reinterpret_cast<const char*>(output.data()),
               static_cast<std::streamsize>(output.size() * sizeof(std::uint16_t)));
    const bool ok = file.good();
    frt_graph_destroy(graph);
    cudaStreamDestroy(stream);
    frt_ctx_destroy(ctx);
    if (!ok) return 1;
    std::cout << "PASS native encoder layer 0\n";
    return 0;
}
