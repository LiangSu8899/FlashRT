...
void test_pi05_runtime_io_adapter() {
    ...
    assert(st.ok_status());
    VisionFrame invalid = image;
    invalid.format = PixelFormat::kBGR8;
    st = io.prepare_vision({invalid});
    assert(st.code == StatusCode::kShapeMismatch);
    invalid = image;
    invalid.image.dtype = DType::kFloat32;
    st = io.prepare_vision({invalid});
    assert(st.code == StatusCode::kShapeMismatch);
    invalid = image;
    invalid.image.layout = Layout::kCHW;
    st = io.prepare_vision({invalid});
    assert(st.code == StatusCode::kShapeMismatch);
    invalid = image;
    invalid.image.shape = Shape{2, 3, 3};
    st = io.prepare_vision({invalid});
    assert(st.code == StatusCode::kShapeMismatch);
    invalid = image;
    ...
    st = io.prepare_vision({invalid});
    assert(st.code == StatusCode::kShapeMismatch);
    st = io.prepare_vision({});
    assert(st.code == StatusCode::kShapeMismatch);
    st = io.prepare_vision({image, image});
    assert(st.code == StatusCode::kShapeMismatch);
