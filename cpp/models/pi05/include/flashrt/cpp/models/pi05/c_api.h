...
/* One calibration observation. `frames` is the complete named camera set for
 * one observation. Dataset and multi-timestamp calibration call observe
 * repeatedly; synchronization, iteration, and decoding remain host policy.
 *
 * `noise` is optional f32 [chunk, 32]. Supplying it makes oracle comparison
 * exact. When omitted, FlashRT generates deterministic normal noise from
 * noise_seed and the current sample index. */
